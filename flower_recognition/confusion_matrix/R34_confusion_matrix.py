...
# 【核心修正】：导入您自己封装的模型结构，确保与训练时绝对镜像对齐
...
def main():
    ...
    model.to(device)
    model.eval()  # 必须开启评估模式，锁定 Dropout 和 BatchNorm
    ...
    plt.title('Confusion Matrix on Validation Set', pad=20, fontsize=16, fontweight='bold')
    ...
    # 9. 输出矢量图与高清位图
    # 【细节修正】：统一输出文件的命名逻辑，并同时输出 pdf 矢量图和 png 高清图
    plt.savefig("R34_confusion_matrix.png", dpi=300, bbox_inches='tight')
    print(">>> 混淆矩阵已生成！保存在当前目录下的 R34_confusion_matrix.pdf 和 R34_confusion_matrix.png")
